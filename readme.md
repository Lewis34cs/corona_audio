...
<input type="file" id="files-8bd223d1-31c6-40fe-bf91-35834087955e" name="files[]" multiple disabled
   style="border:none" />
<output id="result-8bd223d1-31c6-40fe-bf91-35834087955e">
...
```
    ...
    Downloading covid19-radiography-database.zip to /content
     99% 1.13G/1.14G [00:43<00:00, 19.2MB/s]
    100% 1.14G/1.14G [00:43<00:00, 27.9MB/s]
...
```
    ...
    Copying files: 3886 files [00:06, 571.14 files/s]
...
We can definitely see more of the infiltrate in the lung areas of the chest x-ray. These infiltrate areas of the lung can determine whether or not a person has Pneumonia. According to Hosseiny et al., when there is radiographic appearances of multifocal ground glass opacity, linear opacities, and consolidation, these are usually seen in cases of coronavirus type infections, including COVID-19, SARS, and MERS.

### Modeling with CLAHE preprocessed images

Now that we've preprocessed our CXR images and split them into train, test, and validation folders, we create our generators, define class weights and model, and train the model on the preprocessed images. 
...
By using our CLAHE CXR images, we are able to improve our model's acuracy to around 97%, which is 3% better than our model that used the unpreprocessed CXR images. We should also note that this model's recall score for classifying the COVID class is 98%, with only a 2% false negative rate. 
...
In this section, we explore some of the functions in the librosa library, and play around with some example audio found in the Virufy dataset.
...
```
    ...
    '/virufy_data-main/clinical/segmented/pos/pos-0421-093-cough-f-24-6.mp3'
...
```python
# Viewing the model's correlation for the Healthy class using the LimeExplainer function
explain_image(model=viru_model1, generator=viru_train_gen, num_samples=1000, num_feats=5)
```
...
```python
# Viewing the model's correlation for the COVID class using the LimeExplainer function
explain_image(model=viru_model1, generator=viru_train_gen, num_samples=1000, num_feats=5, class_label=1)
```
...
```python
# Extracting the contents of our coughvid zipped folder
...
This dataset required a lot of scrubbing to retrieve the data we needed to work with. Along with using the 'cough_detected' column > 0.8 as a filter for choosing the correct audio to model off of, we also had to remove rows where the labels were null or labeled as 'symptomatic'. The label 'symptomatic' indicated the patient had some symptoms similar to COVID-19, but they had not received a diagnostic result. We also removed rows where there was no cough according to the pulmonologists that reviewed those specific audio files.
...
root_dir = '/cv_audio/'
new_dir = '/cleaned_audio/'

convert_audio(root_dir, new_dir)
```
...
Extending all audio in virufy folder


```python
create_silence(root_dir='/virufy_data-main/clinical/segmented/', 
               new_dir='/ext_audio_virufy/')
```

    Time to run cell: 47 seconds
    

 Creating new silenced audio files from our '/cleaned_audio/' folder
...
We then make sure to save the extended Coughvid audio files into our google drive, because this processing step does take over an hour to complete.
...
spec_root = '/.../spectrograms/pos/'
spec_ex = os.listdir('/.../spectrograms/pos/')[0]
...
spectro_images = num_items('/.../spectrograms', 
                           targets=['/pos', '/neg'])
```
...
# easier access
f_name = '/.../spectro_folders_zip.zip'
with ZipFile(f_name, 'r') as zipf:
      zipf.extractall()
      print('Done')
```
...
```python
base_folder = '/.../spectrograms'

splitfolders.ratio(input=base_folder, 
                    output='/.../spectro_folders', 
                    seed=42, ratio=(0.7, 0.1, 0.2)) # ratio order: train, val, test
```


```python
# Viewing number of images in our folders
...
While our model received a 90.5% accuracy, it classified all images as Healthy. The model was not able to differentiate between the COVID class and the Healthy class.
...
Considering the time constraint for the capstone project, I won't be able to explore all of these options to the fullest extent before the project is due. However, I would say these options of exploration could definitely be deemed as further research.
...
#### Fitting a model using the augmented spectrogram folders
...
When working with images like x-rays or MRI scans, I highly recommend using CLAHE as a preprocessing technique to create new images that give the model more to learn from. CLAHE is able to provide enough contrast to the image without overamplifying the intensity of the pixels. It is a great tool if the goal of your project involves detection and/or recognition between classes.
...
Even though my x-ray model performed extremely well (97% accuracy and 98% recall rate for the COVID class) when being compared to my audio model (88% accuracy, 14% recall rate for the COVID class), we must also consider the risks that come with an infected patient getting x-rayed at a medical facility. COVID-19 is highly contagious and can be easily passed from one person to another. Introducing the infected patient to those who work in the facility, along with those who are there because they are already sick can be dangerous, especially if a person who is exposed has high risk factors for becoming hospitalized from COVID-19. We must also think about the cost of an x-ray. X-rays are not cheap, especially for those who do not have health insurance. While my audio model still needs more work and further exploration, I recommend health companies to invest in obtaining high quality COVID-19 positive audio data with expert-confirmed labels would be the key to helping prevent the further spread of COVID-19, and possibly future variants. 
...
While I have only been able to explore a few different methods when creating spectrograms, I have a lot of different options in front of me in terms of identifying further ways to tackle this problem. Finding other coughing audio through other sources could prove to be beneficial, especially if the audio is high quality with laboratory confirmed labels. Using different parameters when creating the spectrograms could also help the model's recognition ability. Trying other audio imaging techniques like Mel-Frequency Cepstral Coefficients (MFCCs), or even trying feature extraction to try and find key features that impact the detection efficiency when using the model to diagnose the audio could also help the model perform better. We could also look into transfer models and using those as a base for our customized top layers.

Another tactic that I will try will involve setting the audio files to a lower time duration before creating the spectrograms. I've realized that some of the audio (like the virufy audio) is less than 2 seconds in duration. By zero padding 8 seconds to the audio file, there is almost no information the model can really work with. So my idea is to clip the silence off the ends of the audio files, then cut each audio file in half that is greater than 6 seconds in duration. This way, I'd be reducing the width of the spectrograms and giving the models more to look at. 
